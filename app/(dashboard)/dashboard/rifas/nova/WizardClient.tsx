...
import { AlertCircle, ChevronRight, Check, Layout, Settings, Rocket, AlertTriangle } from "lucide-react"
...
export default function NovaRifaPage({ userPlan, hasPixConfig }: NovaRifaPageProps) {
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 pb-20">
            ...
            <div className="space-y-4">
                {!hasPixConfig && (
                    <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200/50 dark:border-amber-800/50 rounded-2xl p-4 flex flex-col sm:flex-row gap-4 items-center justify-between animate-in fade-in slide-in-from-top-4 duration-500">
                        <div className="flex gap-3 items-center">
                            <div className="p-2 bg-amber-100 dark:bg-amber-900/40 rounded-xl">
                                <AlertTriangle className="w-5 h-5 text-amber-600" />
                            </div>
                            <div>
                                <p className="text-sm font-black text-amber-800 dark:text-amber-400">Recebimento via PIX não configurado!</p>
                                <p className="text-[10px] text-amber-700 dark:text-amber-500 font-medium">Sua campanha não poderá receber pagamentos sem a Chave PIX e o QR Code.</p>
                            </div>
                        </div>
                        <Link
                            href="/conta?tab=financeiro#pix-manual"
                            className="bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-amber-600/10 active:scale-95 whitespace-nowrap"
                        >
                            Configurar Agora
                        </Link>
                    </div>
